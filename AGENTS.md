...
- **Presets** (`lib/arc/presets.ts` types + `lib/arc/presets-store.ts` server store + `hooks/use-presets.ts`): full-layout snapshots, operator-saved only (no built-ins), applied via `replaceConfig`. Persisted to a JSON file on disk (`PRESETS_DIR` env, else `<cwd>/.lion-presets`) via `app/api/presets/*` rather than localStorage, so saved layouts are committed with the repo and a fresh clone has them. Switch/save in the toolbar.
